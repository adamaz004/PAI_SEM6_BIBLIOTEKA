...
if (!$connection) {
    ...
    exit();
           ...
           if( $result = mysqli_query(
                $connection,
                "INSERT INTO rent (books_id, user_id, status) VALUES ('$books_id', '$user', 'Zarezerwowane');",
    ...
    if( $result = mysqli_query(
                $connection,
                "Update books set status='Zarezerwowane' where books_id=$books_id",
            ) or die("DB error 2:  $connection->error);"));
else {
            print "<center><h1 style='padding-top:20%; color: red;'>Error - try again!</h1></center>";
            header("Refresh: 2; URL=../index.php");
        }
    print "<center><h1 style='padding-top:20%; color: red;'>Zarezerwowano!</h1></center>";
